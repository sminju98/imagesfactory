 ...
 */
async function enhanceAndTranslatePrompt(prompt) {
...
async function generateWithDALLE3(params) {
    ...
    // API 호출 함수
    const callAPI = async (promptToUse) => {
        return axios_1.default.post('https://api.openai.com/v1/images/generations', {
            model: 'dall-e-3',
            prompt: promptToUse,
            n: 1,
            size,
...
async function generateWithSDXL(params) {
    ...
    return generateWithReplicate({ ...params, prompt: finalPrompt, modelId: 'sdxl' }, 'stability-ai/stable-diffusion-3.5-large', {
        width: width,
        height: height,
...
async function generateWithFlux(params) {
    ...
    return generateWithReplicate({ ...params, prompt: finalPrompt, modelId: 'flux' }, 'black-forest-labs/flux-1.1-pro', {
...
async function generateWithHunyuan(params) {
    ...
    return generateWithReplicate({ ...params, prompt: finalPrompt, modelId: 'hunyuan' }, 'tencent/hunyuan-image-3', {
        aspect_ratio: aspectRatio,
        num_outputs: 1,
    });
}
...
async function generateWithSeedream(params) {
    ...
    const response = await axios_1.default.post('https://api.segmind.com/v1/seedream-4', {
        prompt: finalPrompt,
...
async function generateWithRecraft(params) {
    ...
    return generateWithReplicate({ ...params, prompt: finalPrompt, modelId: 'recraft' }, 'recraft-ai/recraft-v3', {
        size: size,
 ...
 */
async function generateWithPlayground(params) {
    const { width = 1024, height = 1024 } = params;
    // 민감한 키워드 우회 처리
    const sanitizedPrompt = sanitizePrompt(params.prompt);
    let finalPrompt = isKorean(sanitizedPrompt) ? await translatePromptToEnglish(sanitizedPrompt) : sanitizedPrompt;
    ...
    return generateWithReplicate({ ...params, prompt: finalPrompt, modelId: 'playground' }, 'jyoung105/playground-v2.5', {
        width: width,
        height: height,
        guidance_scale: 3,
    });
 ...
 */
async function generateWithKandinsky(params) {
    const { width = 1024, height = 1024 } = params;
    // 민감한 키워드 우회 처리
    const sanitizedPrompt = sanitizePrompt(params.prompt);
    let finalPrompt = isKorean(sanitizedPrompt) ? await translatePromptToEnglish(sanitizedPrompt) : sanitizedPrompt;
    ...
    return generateWithReplicate({ ...params, prompt: finalPrompt, modelId: 'kandinsky' }, 'asiryan/kandinsky-3.0', {
        width: width,
        height: height,
...
async function generateWithRealisticVision(params) {
    ...
    return generateWithReplicate({ ...params, prompt: finalPrompt, modelId: 'realistic-vision' }, '2c8e954decbf70b7607a4414e5785ef9e4de4b8c51d50fb8b8b349160e0ef6bb', {
 ...
 */
async function generateWithGPTImage(params) {
    const { prompt, width = 1024, height = 1024, referenceImageUrl } = params;
    // 먼저 원본 프롬프트로 번역만 수행
    let finalPrompt = isKorean(prompt) ? await translatePromptToEnglish(prompt) : prompt;
    if (referenceImageUrl) {
    ...
    if (!imageUrl && imageData.b64_json) {
        ...
        const { storage } = await Promise.resolve().then(() => __importStar(require('./firestore')));
...
async function generateWithGemini(params) {
    ...
    // API 호출 함수
    const callAPI = async (promptToUse) => {
        return axios_1.default.post('https://generativelanguage.googleapis.com/v1beta/models/imagen-4.0-generate-001:predict', {
            instances: [
                {
                    prompt: promptToUse,
                },
...
async function generateWithIdeogram(params) {
    ...
    return generateWithReplicate({ ...params, prompt: finalPrompt, modelId: 'ideogram' }, 'ideogram-ai/ideogram-v3-turbo', {
        prompt: finalPrompt,
        aspect_ratio: aspectRatio,
    });
}
 ...
 */
async function generateWithMidjourney(params) {
    const { prompt, width = 1024, height = 1024 } = params;
    ...
    while (Date.now() - startTime < maxWaitTime) {
        ...
        const hasSuccessfulSlot = genDetails.processing_result?.slots?.some((slot) => slot.status === 'success' && slot.url);
    ...
    const successfulSlots = slots.filter((slot) => slot.status === 'success' && slot.url);
    if (successfulSlots.length > 0) {
        imageUrls = successfulSlots.map((slot) => slot.url);
        console.log(`🖼️ [Midjourney] slots에서 ${imageUrls.length}장 URL 추출`);
    }
    // 2) images 배열에서 찾기
    if (imageUrls.length === 0 && genDetails?.images?.length > 0) {
        imageUrls = genDetails.images.filter((img) => typeof img === 'string' || img?.url)
