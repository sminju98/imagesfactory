# 기술 명세서 (Technical Specification)
# imagesfactory

**문서 버전**: 1.0  
**작성일**: 2025-11-23  
**대상 독자**: 개발팀, DevOps

---

## 📋 개요

본 문서는 imagesfactory의 기술 아키텍처, 기술 스택, 개발 환경, 배포 전략을 정의합니다.

---

## 🏗 시스템 아키텍처

### 전체 아키텍처 다이어그램

```
┌─────────────┐
│   사용자     │
└──────┬──────┘
       │
       ↓
┌──────────────────────────────────────┐
│         CDN (Cloudflare)             │
│     (정적 파일, 이미지 캐싱)          │
└──────────────┬───────────────────────┘
               │
               ↓
┌──────────────────────────────────────┐
│      Next.js (Vercel/AWS)            │
│  ┌────────────────────────────────┐  │
│  │  Frontend (React)              │  │
│  └────────────────────────────────┘  │
│  ┌────────────────────────────────┐  │
│  │  API Routes (Backend)          │  │
│  └────────────────────────────────┘  │
└───┬────────┬──────────┬──────────┬───┘
    │        │          │          │
    ↓        ↓          ↓          ↓
┌────────┐ ┌─────┐ ┌────────┐ ┌────────┐
│ PostgreSQL │Redis│ AWS S3 │ │AI APIs │
│           │     │        │ │        │
│  (Vercel) │(Up- │(Storage)│ │DALL-E  │
│  Postgres │stash)│        │ │Stable  │
│           │     │        │ │Diffusion│
└───────────┘└─────┘└────────┘ └────────┘
                    │
                    ↓
             ┌─────────────┐
             │   SendGrid  │
             │   (Email)   │
             └─────────────┘
```

### 레이어 아키텍처

```
┌─────────────────────────────────────┐
│    Presentation Layer (UI)          │
│    - Next.js Pages/Components       │
│    - React Hooks                    │
│    - Zustand (State Management)     │
└─────────────────┬───────────────────┘
                  │
┌─────────────────▼───────────────────┐
│    API Layer                        │
│    - Next.js API Routes             │
│    - REST API Endpoints             │
│    - Middleware (Auth, Rate Limit)  │
└─────────────────┬───────────────────┘
                  │
┌─────────────────▼───────────────────┐
│    Business Logic Layer             │
│    - Services                       │
│    - Use Cases                      │
│    - Domain Models                  │
└─────────────────┬───────────────────┘
                  │
┌─────────────────▼───────────────────┐
│    Data Access Layer                │
│    - Prisma ORM                     │
│    - Redis Cache                    │
│    - S3 Client                      │
└─────────────────────────────────────┘
```

---

## 💻 기술 스택

### Frontend

#### Core
- **Next.js**: 14.2+ (App Router)
  - 이유: SSR, API Routes 통합, 최적화
- **React**: 18+
- **TypeScript**: 5.0+
  - 타입 안정성, 개발 생산성

#### UI/UX
- **Tailwind CSS**: 3.4+
  - 빠른 스타일링, 일관성
- **shadcn/ui**: 컴포넌트 라이브러리
  - 접근성, 커스터마이징
- **Radix UI**: 헤드리스 컴포넌트
- **Lucide React**: 아이콘
- **Framer Motion**: 애니메이션

#### 폼 & 검증
- **React Hook Form**: 폼 관리
- **Zod**: 스키마 검증
  - 타입 안전성, 런타임 검증

#### 상태 관리
- **Zustand**: 전역 상태
  - 가볍고 간단
- **TanStack Query (React Query)**: 서버 상태
  - 캐싱, 자동 리패치

#### 기타
- **Axios**: HTTP 클라이언트
- **date-fns**: 날짜 처리
- **react-dropzone**: 파일 업로드
- **recharts**: 차트 (대시보드)

### Backend

#### Core
- **Node.js**: 20 LTS
- **Next.js API Routes**: 서버리스 API
- **TypeScript**: 5.0+

#### 데이터베이스
- **PostgreSQL**: 16+
  - 관계형 데이터, ACID
- **Prisma ORM**: 4.0+
  - 타입 안전, 마이그레이션

#### 캐시 & Queue
- **Redis**: 7+
  - 세션, 캐시, Queue
- **BullMQ**: Queue 관리
  - 백그라운드 작업, 재시도

#### 인증
- **NextAuth.js (Auth.js)**: v5
  - 이메일/구글 로그인 통합
- **bcryptjs**: 비밀번호 해싱
- **jsonwebtoken**: JWT 생성/검증

#### 외부 API
- **OpenAI API**: DALL-E 3
- **Stability AI API**: Stable Diffusion
- **Google Cloud Translation**: 프롬프트 번역
- **SendGrid**: 이메일 전송
- **Toss Payments**: 결제

#### 파일 스토리지
- **AWS S3**: 이미지 저장
- **Sharp**: 이미지 처리 (썸네일)

#### 보안
- **helmet**: HTTP 보안 헤더
- **express-rate-limit**: Rate limiting
- **cors**: CORS 설정
- **validator**: 입력 검증

### DevOps & 인프라

#### 호스팅
- **Vercel**: Frontend + API (우선)
  - 장점: Zero-config, 자동 배포
- **AWS** (대안): EC2, RDS, S3
  - 장점: 더 많은 제어

#### CI/CD
- **GitHub Actions**: 자동화
  - 테스트, 빌드, 배포
- **Vercel Git Integration**: 자동 배포

#### 모니터링
- **Sentry**: 에러 트래킹
- **Vercel Analytics**: 성능 모니터링
- **LogTail** 또는 **Datadog**: 로그 관리

#### 테스팅
- **Jest**: 유닛 테스트
- **React Testing Library**: 컴포넌트 테스트
- **Playwright**: E2E 테스트
- **k6** 또는 **Artillery**: 부하 테스트

---

## 🗂 프로젝트 구조

```
imagesfactory/
├── src/
│   ├── app/                    # Next.js App Router
│   │   ├── (auth)/            # 인증 관련 페이지
│   │   │   ├── login/
│   │   │   ├── signup/
│   │   │   └── layout.tsx
│   │   ├── (dashboard)/       # 로그인 필요 페이지
│   │   │   ├── dashboard/
│   │   │   ├── create/
│   │   │   ├── history/
│   │   │   ├── points/
│   │   │   ├── profile/
│   │   │   └── layout.tsx
│   │   ├── api/               # API Routes
│   │   │   ├── auth/
│   │   │   ├── generate/
│   │   │   ├── points/
│   │   │   ├── payment/
│   │   │   └── user/
│   │   ├── layout.tsx         # Root Layout
│   │   ├── page.tsx           # 랜딩 페이지
│   │   └── globals.css
│   ├── components/            # React 컴포넌트
│   │   ├── ui/               # shadcn/ui 컴포넌트
│   │   ├── auth/             # 인증 컴포넌트
│   │   ├── generation/       # 이미지 생성 컴포넌트
│   │   ├── layout/           # 레이아웃 컴포넌트
│   │   └── shared/           # 공통 컴포넌트
│   ├── lib/                   # 유틸리티 & 설정
│   │   ├── prisma.ts         # Prisma Client
│   │   ├── redis.ts          # Redis Client
│   │   ├── s3.ts             # S3 Client
│   │   ├── auth.ts           # NextAuth 설정
│   │   ├── email.ts          # 이메일 서비스
│   │   └── utils.ts          # 유틸 함수
│   ├── services/              # 비즈니스 로직
│   │   ├── auth.service.ts
│   │   ├── generation.service.ts
│   │   ├── point.service.ts
│   │   ├── payment.service.ts
│   │   └── ai.service.ts
│   ├── workers/               # 백그라운드 작업
│   │   ├── image-generation.worker.ts
│   │   └── email.worker.ts
│   ├── types/                 # TypeScript 타입
│   │   ├── api.ts
│   │   ├── models.ts
│   │   └── index.ts
│   ├── hooks/                 # Custom React Hooks
│   │   ├── useAuth.ts
│   │   ├── useGeneration.ts
│   │   └── usePoints.ts
│   ├── store/                 # Zustand Store
│   │   ├── authStore.ts
│   │   └── uiStore.ts
│   └── middleware.ts          # Next.js Middleware
├── prisma/
│   ├── schema.prisma         # DB 스키마
│   ├── migrations/           # 마이그레이션
│   └── seed.ts               # 시드 데이터
├── public/                    # 정적 파일
│   ├── images/
│   └── icons/
├── tests/                     # 테스트
│   ├── unit/
│   ├── integration/
│   └── e2e/
├── docs/                      # 문서
├── .env.example              # 환경 변수 예시
├── .env.local                # 로컬 환경 변수
├── next.config.js
├── tailwind.config.ts
├── tsconfig.json
├── package.json
└── README.md
```

---

## 🔧 개발 환경 설정

### 필수 소프트웨어
- Node.js: 20 LTS
- pnpm: 8+ (패키지 매니저)
- PostgreSQL: 16+ (로컬 개발)
- Redis: 7+ (로컬 개발)
- Git

### 환경 변수

#### .env.local (개발)
```bash
# Database
DATABASE_URL="postgresql://user:password@localhost:5432/imagesfactory"

# Redis
REDIS_URL="redis://localhost:6379"

# NextAuth
NEXTAUTH_URL="http://localhost:3000"
NEXTAUTH_SECRET="your-secret-key-min-32-chars"

# Google OAuth
GOOGLE_CLIENT_ID="your-google-client-id"
GOOGLE_CLIENT_SECRET="your-google-client-secret"

# AI APIs
OPENAI_API_KEY="sk-..."
STABILITY_API_KEY="sk-..."

# AWS S3
AWS_ACCESS_KEY_ID="your-access-key"
AWS_SECRET_ACCESS_KEY="your-secret-key"
AWS_REGION="ap-northeast-2"
AWS_S3_BUCKET="imagesfactory-dev"

# SendGrid
SENDGRID_API_KEY="SG...."
SENDGRID_FROM_EMAIL="noreply@imagesfactory.com"

# Toss Payments
TOSS_CLIENT_KEY="test_ck_..."
TOSS_SECRET_KEY="test_sk_..."

# Google Translate
GOOGLE_TRANSLATE_API_KEY="your-key"

# Sentry (선택)
SENTRY_DSN="https://..."
```

#### .env.production (프로덕션)
```bash
# 위와 동일하지만 프로덕션 키 사용
DATABASE_URL="postgresql://..."
REDIS_URL="rediss://..."
NEXTAUTH_URL="https://imagesfactory.com"
# ... (프로덕션 키)
```

### 로컬 개발 시작

```bash
# 저장소 클론
git clone https://github.com/your-org/imagesfactory.git
cd imagesfactory

# 의존성 설치
pnpm install

# 환경 변수 설정
cp .env.example .env.local
# .env.local 편집

# 데이터베이스 마이그레이션
pnpm prisma migrate dev

# 시드 데이터 (선택)
pnpm prisma db seed

# 개발 서버 시작
pnpm dev

# Worker 시작 (별도 터미널)
pnpm worker:dev
```

---

## 🎨 API 설계 원칙

### RESTful API
- HTTP 메서드 의미론적 사용
  - GET: 조회
  - POST: 생성
  - PUT/PATCH: 수정
  - DELETE: 삭제
- 명사 기반 URL
  - ✅ `/api/generate`
  - ❌ `/api/createImage`
- 복수형 사용
  - `/api/users` (목록)
  - `/api/users/:id` (단일)

### 응답 형식

#### 성공 응답
```typescript
{
  success: true,
  data: {
    // 실제 데이터
  },
  meta?: {
    page: 1,
    limit: 20,
    total: 100
  }
}
```

#### 에러 응답
```typescript
{
  success: false,
  error: {
    code: "INSUFFICIENT_POINTS",
    message: "포인트가 부족합니다.",
    details?: {
      required: 1000,
      current: 500
    }
  }
}
```

### HTTP 상태 코드
- 200: OK (성공)
- 201: Created (생성 성공)
- 204: No Content (성공, 응답 본문 없음)
- 400: Bad Request (잘못된 요청)
- 401: Unauthorized (인증 필요)
- 403: Forbidden (권한 없음)
- 404: Not Found (리소스 없음)
- 409: Conflict (충돌, 예: 이메일 중복)
- 422: Unprocessable Entity (검증 실패)
- 429: Too Many Requests (Rate limit)
- 500: Internal Server Error (서버 에러)
- 503: Service Unavailable (서비스 불가)

---

## 🔒 보안 설계

### 인증 플로우

#### 이메일 로그인
```
1. 사용자 → POST /api/auth/login (email, password)
2. 서버: 비밀번호 검증 (bcrypt)
3. 서버: JWT 생성 (액세스 + 리프레시)
4. 서버: 리프레시 토큰 Redis 저장
5. 서버 → 클라이언트: 토큰 반환 (HttpOnly 쿠키)
6. 클라이언트: 로컬 스토리지에 사용자 정보 저장
```

#### 구글 로그인
```
1. 사용자 → 구글 로그인 버튼 클릭
2. NextAuth → 구글 OAuth 페이지
3. 사용자: 구글 인증
4. 구글 → NextAuth: 인증 코드
5. NextAuth → 구글: 토큰 교환
6. NextAuth: 사용자 정보 가져오기
7. NextAuth: DB 조회/생성
8. NextAuth: 세션 생성
9. 클라이언트: 대시보드 리다이렉트
```

#### 토큰 갱신
```
1. 액세스 토큰 만료 (401 에러)
2. 클라이언트 → POST /api/auth/refresh (리프레시 토큰)
3. 서버: 리프레시 토큰 검증 (Redis)
4. 서버: 새 액세스 토큰 발급
5. 서버 → 클라이언트: 새 토큰
6. 클라이언트: 원래 요청 재시도
```

### 보안 조치

#### 1. Input Validation (입력 검증)
```typescript
// Zod 스키마 예시
const signupSchema = z.object({
  email: z.string().email(),
  password: z.string()
    .min(8, "최소 8자")
    .regex(/[a-zA-Z]/, "영문 포함")
    .regex(/[0-9]/, "숫자 포함"),
  name: z.string().min(2).max(50),
});
```

#### 2. XSS 방지
- React: 자동 이스케이핑
- DOMPurify: HTML 콘텐츠 sanitize
- Content Security Policy 헤더

#### 3. CSRF 방지
- NextAuth: CSRF 토큰 자동 처리
- SameSite 쿠키 설정

#### 4. SQL Injection 방지
- Prisma ORM: Parameterized queries

#### 5. Rate Limiting
```typescript
// 예시: express-rate-limit
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15분
  max: 100, // 최대 100 요청
  message: "너무 많은 요청입니다.",
});
```

#### 6. 프롬프트 필터링
```typescript
const BANNED_WORDS = [
  "욕설", "폭력", "성적", // ...
];

function validatePrompt(prompt: string): boolean {
  const lowerPrompt = prompt.toLowerCase();
  return !BANNED_WORDS.some(word => 
    lowerPrompt.includes(word)
  );
}
```

---

## 📡 이미지 생성 워크플로우

### Queue 기반 처리

```
┌─────────────┐
│  사용자 요청  │
└──────┬──────┘
       │
       ↓
┌──────────────────┐
│  API Route       │
│  1. 검증          │
│  2. 포인트 차감   │
│  3. DB 레코드 생성│
└──────┬───────────┘
       │
       ↓
┌──────────────────┐
│  BullMQ Queue    │
│  작업 추가        │
└──────┬───────────┘
       │
       ↓
┌──────────────────┐
│  Worker          │
│  백그라운드 처리  │
└──────┬───────────┘
       │
       ↓ (반복)
┌─────────────────────────┐
│ AI API 호출 (1장씩)      │
│ 1. DALL-E/SD API 호출   │
│ 2. 이미지 다운로드        │
│ 3. S3 업로드             │
│ 4. 썸네일 생성            │
│ 5. DB 저장               │
│ 6. 진행률 업데이트        │
└──────┬──────────────────┘
       │
       ↓ (모두 완료)
┌──────────────────┐
│  1. ZIP 생성     │
│  2. S3 업로드    │
│  3. 이메일 발송   │
│  4. 상태 완료 변경│
└──────────────────┘
```

### Worker 구현 예시

```typescript
// workers/image-generation.worker.ts
import { Worker, Job } from 'bullmq';
import { generateImage } from '@/services/ai.service';
import { uploadToS3 } from '@/lib/s3';
import { prisma } from '@/lib/prisma';

interface GenerationJob {
  generationId: string;
  prompt: string;
  imageCount: number;
  style: string;
  aspectRatio: string;
}

const worker = new Worker(
  'image-generation',
  async (job: Job<GenerationJob>) => {
    const { generationId, prompt, imageCount, style, aspectRatio } = job.data;
    
    try {
      // 상태 업데이트: PROCESSING
      await prisma.imageGeneration.update({
        where: { id: generationId },
        data: { status: 'PROCESSING' },
      });
      
      for (let i = 0; i < imageCount; i++) {
        // 이미지 생성
        const imageBuffer = await generateImage({
          prompt,
          style,
          aspectRatio,
        });
        
        // S3 업로드
        const imageUrl = await uploadToS3(imageBuffer, `${generationId}/${i}.png`);
        
        // 썸네일 생성
        const thumbnailBuffer = await createThumbnail(imageBuffer);
        const thumbnailUrl = await uploadToS3(thumbnailBuffer, `${generationId}/${i}_thumb.png`);
        
        // DB 저장
        await prisma.image.create({
          data: {
            generationId,
            imageUrl,
            thumbnailUrl,
            order: i + 1,
            width: 1024,
            height: 1024,
          },
        });
        
        // 진행률 업데이트
        const progress = Math.round(((i + 1) / imageCount) * 100);
        await prisma.imageGeneration.update({
          where: { id: generationId },
          data: { progress },
        });
        
        // Job progress 업데이트 (BullMQ)
        await job.updateProgress(progress);
      }
      
      // ZIP 생성 및 이메일 발송
      await createZipAndSendEmail(generationId);
      
      // 상태 업데이트: COMPLETED
      await prisma.imageGeneration.update({
        where: { id: generationId },
        data: { 
          status: 'COMPLETED',
          completedAt: new Date(),
        },
      });
      
    } catch (error) {
      // 실패 처리
      await prisma.imageGeneration.update({
        where: { id: generationId },
        data: { 
          status: 'FAILED',
          failedReason: error.message,
        },
      });
      
      // 포인트 환불
      await refundPoints(generationId);
      
      throw error;
    }
  },
  {
    connection: redisConnection,
    concurrency: 5, // 동시 처리 수
    limiter: {
      max: 10, // 최대 10개
      duration: 1000, // 1초당
    },
  }
);

worker.on('completed', (job) => {
  console.log(`Job ${job.id} completed`);
});

worker.on('failed', (job, err) => {
  console.error(`Job ${job?.id} failed:`, err);
});
```

---

## 📊 데이터베이스 최적화

### 인덱스 전략

```prisma
// User
@@index([email]) // 로그인 조회
@@index([provider, providerId]) // 소셜 로그인

// ImageGeneration
@@index([userId, createdAt(sort: Desc)]) // 히스토리 조회
@@index([status]) // 상태별 조회
@@index([createdAt]) // 날짜 범위 조회

// Image
@@index([generationId]) // 생성별 이미지 조회

// PointTransaction
@@index([userId, createdAt(sort: Desc)]) // 거래 내역
@@index([type]) // 유형별 조회

// Payment
@@index([userId, createdAt(sort: Desc)]) // 결제 내역
@@index([status]) // 상태별 조회
@@index([paymentKey]) // 토스 조회
```

### 쿼리 최적화

#### N+1 문제 방지
```typescript
// ❌ 나쁜 예: N+1 쿼리
const generations = await prisma.imageGeneration.findMany({
  where: { userId },
});
for (const gen of generations) {
  const images = await prisma.image.findMany({
    where: { generationId: gen.id },
  });
}

// ✅ 좋은 예: include로 한 번에
const generations = await prisma.imageGeneration.findMany({
  where: { userId },
  include: {
    images: true,
  },
});
```

#### 페이지네이션
```typescript
// Cursor 기반 (권장)
const generations = await prisma.imageGeneration.findMany({
  take: 20,
  skip: cursor ? 1 : 0,
  cursor: cursor ? { id: cursor } : undefined,
  orderBy: { createdAt: 'desc' },
});

// Offset 기반 (단순)
const generations = await prisma.imageGeneration.findMany({
  take: 20,
  skip: (page - 1) * 20,
  orderBy: { createdAt: 'desc' },
});
```

### 캐싱 전략

#### Redis 캐시
```typescript
// 포인트 잔액 캐싱
async function getPoints(userId: string): Promise<number> {
  const cacheKey = `points:${userId}`;
  
  // 캐시 확인
  const cached = await redis.get(cacheKey);
  if (cached) return parseInt(cached);
  
  // DB 조회
  const user = await prisma.user.findUnique({
    where: { id: userId },
    select: { points: true },
  });
  
  // 캐시 저장 (1시간)
  await redis.setex(cacheKey, 3600, user.points);
  
  return user.points;
}

// 포인트 업데이트 시 캐시 무효화
async function updatePoints(userId: string, amount: number) {
  await prisma.user.update({
    where: { id: userId },
    data: { points: { increment: amount } },
  });
  
  // 캐시 삭제
  await redis.del(`points:${userId}`);
}
```

---

## 🚀 배포 전략

### Vercel 배포 (권장)

#### 설정
```javascript
// next.config.js
module.exports = {
  images: {
    domains: ['imagesfactory.s3.amazonaws.com'],
  },
  experimental: {
    serverActions: true,
  },
  env: {
    NEXT_PUBLIC_APP_URL: process.env.NEXTAUTH_URL,
  },
};
```

#### 환경별 배포
- **Production**: `main` 브랜치 푸시 시 자동 배포
- **Preview**: PR 생성 시 프리뷰 환경 자동 생성
- **Development**: `dev` 브랜치

#### Vercel 설정
```json
{
  "buildCommand": "pnpm build",
  "outputDirectory": ".next",
  "installCommand": "pnpm install",
  "devCommand": "pnpm dev",
  "env": {
    // 환경 변수 (Vercel 대시보드에서 설정)
  }
}
```

### AWS 배포 (대안)

#### 아키텍처
```
┌──────────────┐
│ CloudFront   │ (CDN)
└──────┬───────┘
       │
┌──────▼───────┐
│ ALB          │ (로드 밸런서)
└──────┬───────┘
       │
┌──────▼───────┐
│ EC2 (x2)     │ (Auto Scaling)
│ - Next.js    │
│ - PM2        │
└──────┬───────┘
       │
┌──────▼───────┬──────────┬─────────┐
│ RDS          │ Elastic- │  S3     │
│ (PostgreSQL) │  Cache   │         │
│              │ (Redis)  │         │
└──────────────┴──────────┴─────────┘
```

#### EC2 설정
```bash
# Node.js 설치
curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -
sudo apt-get install -y nodejs

# pnpm 설치
npm install -g pnpm

# PM2 설치
npm install -g pm2

# 프로젝트 클론 및 빌드
git clone https://github.com/your-org/imagesfactory.git
cd imagesfactory
pnpm install
pnpm build

# PM2로 실행
pm2 start npm --name "imagesfactory" -- start
pm2 save
pm2 startup
```

---

## 🧪 테스트 전략

### 테스트 피라미드
```
      ┌────────┐
      │   E2E  │ (10%)
      │        │
      ├────────┤
      │Integration│ (30%)
      │   Tests    │
      ├────────────┤
      │Unit Tests  │ (60%)
      │            │
      └────────────┘
```

### 유닛 테스트 예시
```typescript
// tests/unit/services/point.service.test.ts
import { describe, it, expect, beforeEach } from 'jest';
import { deductPoints, refundPoints } from '@/services/point.service';

describe('PointService', () => {
  describe('deductPoints', () => {
    it('포인트 차감 성공', async () => {
      const userId = 'test-user';
      const amount = 500;
      
      const result = await deductPoints(userId, amount, '테스트');
      
      expect(result.success).toBe(true);
      expect(result.newBalance).toBe(500); // 1000 - 500
    });
    
    it('포인트 부족 시 실패', async () => {
      const userId = 'test-user';
      const amount = 2000;
      
      await expect(
        deductPoints(userId, amount, '테스트')
      ).rejects.toThrow('포인트가 부족합니다');
    });
  });
});
```

### 통합 테스트 예시
```typescript
// tests/integration/api/generate.test.ts
import { describe, it, expect } from 'jest';
import request from 'supertest';
import { app } from '@/app';

describe('POST /api/generate', () => {
  it('이미지 생성 요청 성공', async () => {
    const response = await request(app)
      .post('/api/generate')
      .set('Authorization', `Bearer ${testToken}`)
      .send({
        prompt: 'a beautiful sunset',
        imageCount: 5,
        style: 'realistic',
        aspectRatio: '16:9',
      });
    
    expect(response.status).toBe(201);
    expect(response.body.success).toBe(true);
    expect(response.body.data.generation).toBeDefined();
  });
});
```

### E2E 테스트 예시
```typescript
// tests/e2e/image-generation.spec.ts
import { test, expect } from '@playwright/test';

test('이미지 생성 플로우', async ({ page }) => {
  // 로그인
  await page.goto('/login');
  await page.fill('input[name="email"]', 'test@example.com');
  await page.fill('input[name="password"]', 'password123');
  await page.click('button[type="submit"]');
  
  // 대시보드 확인
  await expect(page).toHaveURL('/dashboard');
  
  // 생성 페이지로 이동
  await page.click('a[href="/create"]');
  
  // 프롬프트 입력
  await page.fill('textarea[name="prompt"]', 'a beautiful sunset');
  await page.selectOption('select[name="style"]', 'realistic');
  await page.fill('input[name="imageCount"]', '5');
  
  // 생성 요청
  await page.click('button:has-text("생성하기")');
  
  // 진행 화면 확인
  await expect(page.locator('text=생성 중')).toBeVisible();
  
  // 완료 대기 (최대 5분)
  await page.waitForSelector('text=완료', { timeout: 300000 });
});
```

---

## 📈 모니터링 & 로깅

### Sentry 설정
```typescript
// sentry.client.config.ts
import * as Sentry from '@sentry/nextjs';

Sentry.init({
  dsn: process.env.SENTRY_DSN,
  environment: process.env.NODE_ENV,
  tracesSampleRate: 1.0,
  beforeSend(event, hint) {
    // 민감한 정보 필터링
    if (event.request?.headers) {
      delete event.request.headers['authorization'];
    }
    return event;
  },
});
```

### 로그 레벨
```typescript
enum LogLevel {
  ERROR = 'error',
  WARN = 'warn',
  INFO = 'info',
  DEBUG = 'debug',
}

// 프로덕션: INFO 이상만
// 개발: DEBUG 모두
```

### 모니터링 메트릭
- API 응답 시간 (P50, P95, P99)
- 에러율
- 이미지 생성 성공률
- Queue 대기 시간
- 데이터베이스 쿼리 시간
- 포인트 거래량
- 결제 성공률

---

## 📝 코드 품질

### ESLint 설정
```javascript
// .eslintrc.js
module.exports = {
  extends: [
    'next/core-web-vitals',
    'plugin:@typescript-eslint/recommended',
    'prettier',
  ],
  rules: {
    '@typescript-eslint/no-unused-vars': 'error',
    '@typescript-eslint/no-explicit-any': 'warn',
    'no-console': 'warn',
  },
};
```

### Prettier 설정
```javascript
// .prettierrc.js
module.exports = {
  semi: true,
  trailingComma: 'es5',
  singleQuote: true,
  printWidth: 100,
  tabWidth: 2,
};
```

### Husky (Git Hooks)
```json
// package.json
{
  "husky": {
    "hooks": {
      "pre-commit": "lint-staged",
      "pre-push": "pnpm test"
    }
  },
  "lint-staged": {
    "*.{ts,tsx}": [
      "eslint --fix",
      "prettier --write"
    ]
  }
}
```

---

**문서 승인**
- [ ] Tech Lead
- [ ] DevOps
- [ ] Security Team

**문서 히스토리**
- v1.0 (2025-11-23): 초안 작성

