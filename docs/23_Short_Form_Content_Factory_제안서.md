...
### STEP 6: 자막 생성 + 타이밍 (Whisper + GPT)
- **Whisper API로 TTS 오디오에서 정확한 타이밍 추출** (우선)
- GPT-5-mini로 문장 분할 최적화 (폴백)
- SRT/ASS 형식 지원
- **사용자가 자막 텍스트/타이밍 수정**
...
| 자막 타이밍 | **Whisper** | 음성→타이밍 추출 | 정확도 높음 |
| 자막 폴백 | GPT-5-mini | 문장 분할 | Whisper 실패 시 |
...
### 자막 타이밍 전략 (Whisper 우선)
1. **Whisper API**: TTS 오디오 분석 → 정확한 단어/문장 타이밍 추출
2. **GPT 폴백**: Whisper 실패 시 텍스트 기반 예측
3. 각 문장 2-4초 길이
4. 한국어: 초당 3-4음절 기준

### Whisper 사용 조건
- TTS 오디오 URL이 HTTP URL일 때 (data: URL 제외)
- 오디오 파일 다운로드 가능할 때
